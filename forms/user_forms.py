...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField(
        ...
        validators=[
            DataRequired(),
            EqualTo("password_confirm", message="Las contraseñas deben coincidir"),
        ],
    )
...
class ProfileForm(FlaskForm):
    ...
    first_name = StringField(
        "Nombre", validators=[DataRequired(), Length(min=3, max=30)]
    )
    last_name = StringField(
        "Apellidos", validators=[DataRequired(), Length(min=10, max=40)]
    )
    image = FileField(
        "Imagen de Perfil",
